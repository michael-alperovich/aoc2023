...
class Position:
    ...
    def __init__(self, graph: List[List[int]], i: int, j: int, direction: Directions, cost: int = 0, smc: int = 0):
        self._graph = graph
        self.i = i
        self.j = j
    ...
    def get_next_positions(self) -> List['Position']:
        ...
        for next_direction in POSSIBLE_DIRECTIONS[self.direction]:
            ...
            if not (0 <= next_i < len(self._graph)
                    and 0 <= next_j < len(self._graph[0])
                    and next_smc <= MAX_STRAIGHT_LENGTH):
                continue
...
def main():
    ...
    best_cost = {}
    heap = [Position(graph, 0, 0, Directions.RIGHT)]
    while len(heap) > 0:
        pos = heapq.heappop(heap)
        if pos.i == len(graph) - 1 and pos.j == len(graph[0]) - 1:
